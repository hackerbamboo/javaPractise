...
class Date {
  ...
  public static boolean isValidDate(int month, int day, int year) {
    if (month > 0 && month < 13 && year > 0 && day <= daysInMonth(month, year) && day >0) {
      return true;
    }
    else{
      System.exit(0);
  ...
  public boolean isBefore(Date d) {
      if (d.year == this.year) {
        if (d.month == this.month) {
          ...
          if (d.day == 1 && this.day == daysInMonth(this.month, this.year)) {
            return true;
          }
          return false;
        }
        ...
        if (d.year == this.year + 1 && d.month == 1 && this.month == 12) {
          if (d.day == 1 && this.day == daysInMonth(this.month, this.year)) {
            return true;
          }
          return false;
  ...
  public boolean isAfter(Date d) {
    return d.isBefore(new Date(this.month, this.day, this.year));
  }
  ...
  public int dayInYear() {
      ...
      result += this.day;
      System.out.println("dayInYear test: " + this.toString() + " nth day is: " + result);
      return result;
     
  }

    /* Returns the total days in this year. 
    *  @return the total days in this year.
    */
    ...
    System.out.println("=========in=======");
      int result = 0;
      if (d.year == this.year) {
        result = this.dayInYear() - d.dayInYear();
      }
      else{
        if (d.year > this.year) {
          result = totalDaysInYear(this.year) - this.dayInYear();
          for (int i = this.year+1; i < d.year; i++) {
            result += totalDaysInYear(i);
          }
          result += d.dayInYear();
          result = result*(-1);
        }
        else if(d.year < this.year){
          result = totalDaysInYear(d.year) - d.dayInYear();
